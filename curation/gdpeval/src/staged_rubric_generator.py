...
async def generate_staged_rubric_v3(
    ...
    if reference_file_urls:
        ref_summary = f"{len(reference_file_urls)} reference files available:\n" + "\n".join([f"- {url}" for url in reference_file_urls])
    else:
...
1. **Use new API**: `def evaluate(workflow, context)` - Access files via `context.get_primary_output()` and `context.files.*`
2. **Check Resource types**: Use `.is_spreadsheet`, `.is_document`, `.is_text_format` properties
3. **Stage 1 = LLM judges to mandate structure** - DO NOT write Stage 1 code with exact column names!
4. **Stage 2 code rules use FLEXIBLE matching** - substring, .lower(), fuzzy matching to find what Stage 1 mandated
5. **Stage 2 code rules CAN check correctness** - bounds, unit tests, consistency checks are encouraged!
6. **Leverage the shape** - Stage 2 can be simple because Stage 1 enforced structure
