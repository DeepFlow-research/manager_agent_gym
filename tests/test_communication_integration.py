...
async def test_thread_multicast_and_grouping() -> None:
    ...
    views = svc.get_all_messages_grouped(grouping=MessageGrouping.BY_THREAD)
    # views is list[ThreadMessagesView]

    assert any(
        v.thread_id == thread.thread_id and v.total_messages >= 1
        for v in cast(list[ThreadMessagesView], views)
    )
