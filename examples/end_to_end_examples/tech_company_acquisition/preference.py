...
def create_tech_acquisition_integration_preferences() -> PreferenceWeights:
    ...
    def _validate_cost_realism(workflow: Workflow, context) -> float:
        ...
        if total_actual < expected_min_cost:
            return 0.0  # Tech acquisitions should cost >$50k
    ...
    compliance_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt=(
                ...
                - demonstrates response to due diligence gaps and valuation disputes
                - shows handling of talent retention issues and technology risks
                - demonstrates preparation for competitive bidding and deal breakage scenarios
                - shows integration challenge management and cultural mismatch mitigation
    ...
    goal_achievement_rubrics = [
        ...
        WorkflowRubric(
            ...
            llm_prompt=(
                ...
                "platform compatibility assessment completed, API integration design finalized, security infrastructure plan established, and data migration strategy defined? "
                "Return 20.0 if technology integration strategy is ready for execution, removing 4.0 for every missing element to a minimum of 0.0."
            ),
            max_score=20.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            ...
            llm_prompt=(
                ...
                "cultural integration plan documented, compensation harmonization strategy established, and leadership transition roadmap created? "
                "Return 15.0 if talent retention strategy is fully developed, removing 2.0 for every missing element to a minimum of 0.0."
            ),
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            ...
            llm_prompt=(
                ...
                "service continuity plan prepared, account management transition procedures defined, and customer communication framework established? "
                "Return 15.0 if all components exist, 6 if there is evidence at a task / resource level of this work being planned / attempted but not completed fully, 0.0 otherwise."
            ),
            max_score=15.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            ...
            llm_prompt=(
                ...
                "HSR filing packages prepared, data privacy compliance plan established (GDPR/CCPA), and cross-border requirements analysis completed? "
                "Return 12.0 if regulatory compliance framework is ready for execution, removing 2.0 for every missing element to a minimum of 0.0."
            ),
            max_score=12.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        ...
        WorkflowRubric(
            ...
            llm_prompt=(
                ...
                "Return 10.0 if technology due diligence is complete, removing 3.0 for every missing element to a minimum of 0.0."
            ),
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            ...
            llm_prompt=(
                ...
                "Return 10.0 if business and financial validation is complete, 5 if there is evidence at a task / resource level of this work being planned / attempted but not completed fully, 0.0 otherwise."
            ),
            max_score=10.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        ...
        WorkflowRubric(
            ...
            llm_prompt=(
                ...
                "Return 6.0 if there is evidence of a plan to preserve customer relationships, 3.0 if there is evidence of a plan but it is not complete, 0.0 otherwise."
            ),
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        WorkflowRubric(
            ...
            llm_prompt=(
                ...
                "data migration completed securely, "
                "security protocols maintained, data integrity verified, and privacy compliance ensured? "
                "Return 6.0 if there is evidence of a plan to migrate data securely, 3.0 if there is evidence of a plan but it is not complete, 0.0 otherwise."
            ),
            max_score=6.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
        ...
        WorkflowRubric(
            ...
            llm_prompt=(
                ...
                "IP ownership fully validated, "
                "software licensing confirmed, patent portfolio transferred, and intellectual property rights secured? "
                "Return 4.0 if there is evidence of a plan to transfer IP ownership, 2.0 if there is evidence of a plan but it is not complete, 0.0 otherwise."
            ),
            max_score=4.0,
            run_condition=RunCondition.ON_COMPLETION,
        ),
