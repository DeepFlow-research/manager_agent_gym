...
from manager_agent_gym.core.common.schemas.llm_responses import (
    LLMScoredResponse,
    LLMScoreLevel,
)
...
from manager_agent_gym.core.evaluation.schemas.success_criteria import ValidationMeta
...
"""
    ...
    def _prepare_scoped_workflow_context(
        ...
        if scope is None:
            return WorkflowDisplay.pretty_print(workflow, include_resources=True)
        ...
        return (
            "\n".join(lines)
            if lines
            else WorkflowDisplay.pretty_print(workflow, include_resources=True)
        )
