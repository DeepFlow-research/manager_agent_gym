...
class RandomManagerAgentV2(ManagerAgent):
    ...
    async def take_action(self, observation: ManagerObservation) -> BaseManagerAction:
        try:
            ...
            action = parse_action_response(json.dumps(envelope), [selected_class])
            return action
...
class OneShotDelegateManagerAgent(ManagerAgent):
    ...
    async def take_action(self, observation: ManagerObservation) -> BaseManagerAction:
        await asyncio.sleep(0.5)
        if not self._has_delegated:
            # Choose an agent if available, otherwise no-op
            agent_id = (
                observation.available_agent_metadata[0]
                if observation.available_agent_metadata
                else None
            )
            self._has_delegated = True
            return AssignAllPendingTasksAction(
                reasoning="Delegate all pending tasks once for baseline",
                agent_id=agent_id.agent_id if agent_id else None,
                success=True,
                result_summary="delegated all pending tasks",
            )
