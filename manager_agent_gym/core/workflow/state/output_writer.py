...
if TYPE_CHECKING:
    ...
    from manager_agent_gym.core.agents.manager_agent.common.interface import (
        ManagerAgent,
    )
...
class WorkflowSerialiser:
    ...
    def save_timestep(
        ...
        stakeholder_state: dict | None,
    ) -> None:
        """Write timestep result and a full workflow snapshot for this timestep.

        Args:
            timestep_result: Result from this timestep
            workflow: Current workflow state
            current_timestep: Timestep number
            manager_agent: Manager agent (for action history)
            stakeholder_state: Serializable stakeholder state dict (from get_serializable_state)
        """
        ...
        try:
                ...
                if manager_agent is not None:
                    ...
                    manager_state = {
                        ...
                        "current_preference_weights": (
                            stakeholder_state.get("weights", {})
                            if stakeholder_state is not None
                            else None
                        ),
                    }
            except Exception:
            ...
            try:
                # Stakeholder state is already serializable dict from get_serializable_state()
                stakeholder_state_serialized = (
                    stakeholder_state if stakeholder_state is not None else None
                )
    ...
    def save_pre_execution_phase(
        self,
        workflow: Workflow,
    ) -> None:
        """Save pre-execution phase logs and summary.

        Writes strongly typed logs from workflow.metadata['pre_execution_logs']
        along with final preference state and rubric summaries.

        Args:
            workflow: Workflow after pre-execution phase
            preferences: Final preference weights with generated rubrics
        """
        try:
            # Get strongly typed logs from metadata
            pre_exec_logs = workflow.metadata.get("pre_execution_logs", {})
            ...
            if wf_dir is not None:
                wf_dir.mkdir(parents=True, exist_ok=True)
                pre_exec_file = (
                    wf_dir / f"pre_execution_{self.output_config.run_id}.json"
                )
                with open(pre_exec_file, "w") as f:
                    json.dump(pre_exec_logs, f, indent=2, default=str)
                logger.info(f"Saved pre-execution snapshot to {pre_exec_file}")

        except Exception:
            logger.error("failed writing pre-execution snapshot", exc_info=True)
        ...
        try:
            ...
            snapshot = {
                **workflow.model_dump(mode="json", exclude={"agents"}),
                "agents": agents_serialized,
                "is_complete": WorkflowQueries.is_complete(workflow),
