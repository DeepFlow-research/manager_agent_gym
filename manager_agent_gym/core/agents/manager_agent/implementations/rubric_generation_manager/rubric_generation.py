...
class EvaluationStageSpec(BaseModel):
    ...
    rules: list[CodeRule | LLMJudgeRule] = Field(
        min_length=1, description="Rules evaluated in this stage"
    )
    ...
    @model_validator(mode="after")
    def validate_min_score(self) -> "EvaluationStageSpec":
...
class ManagerAgentGeneratedStagedRubric(BaseModel):
    ...
    category_name: str = Field(description="High-level category name for this rubric")
    rationale: str | None = Field(
    ...
    stages: list[EvaluationStageSpec] = Field(
        min_length=1, description="Evaluation stages in sequential order"
    )
